...
describe('Persist and Restore', function () {
  ...
  describe('#Instance', async function () {
    ...
    it('Persist and Restore', async function () {
        let data = instanceA.toString();
        let instanceB = new BalanceGroup('instanceB');
        instanceB.fromString(data);
        for(let i=0;i<instanceB._ledger.length;i++) {
          assert.equal(instanceB._ledger[i].sum,instanceA._ledger[i].sum);
        }
    });
  });
  //
